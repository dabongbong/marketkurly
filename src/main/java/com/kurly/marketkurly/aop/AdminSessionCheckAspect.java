...
public class AdminSessionCheckAspect {
	...
	public Object sessionCheck(ProceedingJoinPoint joinPoint) throws Throwable{
		...
		//HttpServletRequest 가 몇번째에 있는지 알수가 없으므로, 반복문으로 끄집어 내자...
		...
		if(
			...
			if(request !=null) {
				...
				if(session.getAttribute("admin")==null) {
					//로그인 에러페이지 보여주기..
						//throw new AdminException("관리자 로그인이 필요한 서비스입니다");
					ModelAndView mav = new ModelAndView();
					result = mav;
					AdminException adminException = new AdminException("관리자 로그인이 필요한 서비스입니다.");
					mav.addObject("e", adminException);
					mav.setViewName("admin/error/result");
				}else {
					result=joinPoint.proceed(); //원래 호출하려면 target의 메서드 호출(가던길 가게함...)
				}
			}
		}
